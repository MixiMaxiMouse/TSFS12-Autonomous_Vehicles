...
class MinBrier(Metric):
    ...
    def update(
        ...
        """
        assert prob is not None, "Probabilistic criterion requires" " the probability of the predictions."
        assert pred.dim() == 4, "The predictions must be multi-modal."

        pred, best_idx = filter_prediction(pred, trg, mask, prob, min_criterion, best_idx, mode_first=mode_first)

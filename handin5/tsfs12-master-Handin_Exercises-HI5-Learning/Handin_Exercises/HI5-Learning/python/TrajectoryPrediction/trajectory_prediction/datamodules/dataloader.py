...
from trajectory_prediction.datamodules.dataset import DroneDataset
from trajectory_prediction.utils import import_from_module
...
class DroneDataModule(LightningDataModule):
    ...
    def __init__(self, config: dict, args: Namespace) -> None:
        ...
        if config["transform"] is not None:
            ...
            if isinstance(config["transform"], list):
                self.transform = T.Compose(
                    [
                        import_from_module("trajectory_prediction.datamodules.transforms", t)()
                        for t in config["transform"]
                    ]
                )
            else:
                self.transform = import_from_module(
                    "trajectory_prediction.datamodules.transforms", config["transform"]
                )()
        ...
        # check if args has eval_only attribute
        if hasattr(args, "eval_only"):
            # to make sure that train and val sets are not required for evaluation
            self.eval_only = True
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        self.train = DroneDataset(
            ...
            split="train" if not self.eval_only else "test",
            transform=self.transform,
            small_data=self.small_data,
        )
        self.val = DroneDataset(
            root=self.root,
            dataset=self.dataset,
            split="val" if not self.eval_only else "test",
            transform=self.transform,
            small_data=self.small_data,
        )
        self.test = DroneDataset(
            root=self.root, dataset=self.dataset, split="test", transform=self.transform, small_data=self.small_data
        )
...
if __name__ == "__main__":
    ...
    config = {"root": "../datasets", "name": "sinD", "batch_size": 32, "transform": "CoordinateTransform"}
    ...
    map_edge_index, map_edge_type = subgraph(map_batch, map_edge_index, map_edge_type, relabel_nodes=True)

    #
    for i in range(map_edge_index.shape[1]):
        if map_edge_type[i] == 2:
            edge = map_edge_index[:, i]
            plt.plot(map_pos[edge, 0], map_pos[edge, 1], color="gray", lw=1, zorder=1, alpha=0.9, linestyle="solid")

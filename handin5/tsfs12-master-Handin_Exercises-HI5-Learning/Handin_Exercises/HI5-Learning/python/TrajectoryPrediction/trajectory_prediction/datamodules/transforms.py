...
class CoordinateTransform(BaseTransform):
    ...
    def __call__(self, data: HeteroData) -> HeteroData:
        ...
        if self.train:
            ta_index = torch.randint(0, hist_pos.size(0), (1,)).item()
        else:
...
class DownSample(BaseTransform):
    """
    Downsample the sequence of the agent.
    """
    ...
    def __call__(self, data: HeteroData) -> HeteroData:
        ...
        sa_mask = data['agent']['sa_mask']
        ma_mask = data['agent']['ma_mask']
        valid_mask = data['agent']['valid_mask']

        data['agent']['inp_pos'] = hist_pos[:, ::2]
        data['agent']['inp_vel'] = hist_vel[:, ::2]

        if 'inp_acc' in data['agent']:
            hist_acc = data['agent']['inp_acc']
            data['agent']['inp_acc'] = hist_acc[:, ::2]

        data['agent']['inp_yaw'] = hist_ori[:, ::2]

        data['agent']['trg_pos'] = fut_pos[:, 0:-1:2]
        data['agent']['trg_vel'] = fut_vel[:, 0:-1:2]
        data['agent']['trg_yaw'] = fut_ori[:, 0:-1:2]

        data['agent']['input_mask'] = input_mask[:, ::2]
        data['agent']['sa_mask'] = sa_mask[:, 0:-1:2]
        data['agent']['ma_mask'] = ma_mask[:, 0:-1:2]
        data['agent']['valid_mask'] = valid_mask[:, 0:-1:2]

        # data['map_point'] = {}
        # data['map_point', 'to', 'map_point'] = {}

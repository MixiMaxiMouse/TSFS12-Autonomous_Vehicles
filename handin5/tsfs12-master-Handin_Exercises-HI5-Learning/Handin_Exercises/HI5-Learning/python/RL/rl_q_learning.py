...
eps = 0.5  # Epsilon-greedy parameter, changed for q 4.13

deacreasing_eps = False  # Whether to decrease epsilon over episodes
...
def select_eps_greedy(s_curr, itr_nbr, Q, params, decreasing_eps=deacreasing_eps):
...
for k in range(nbr_iters):
    ...
    while not terminal_state:
        ...
        Q[s_curr[0], s_curr[1], action] = Q[s_curr[0], s_curr[1], action] + alpha * (
            r + gamma * np.max(Q[s_prim[0], s_prim[1]]) - Q[s_curr[0], s_curr[1], action]
        )
...
plt.title(f"Sum of rewards for each episode (non decreasing epsilon) (average over {N})")
plt.savefig("RL_Q_learning_rewards_non_decreasing_eps.pdf", dpi=300)
despine()

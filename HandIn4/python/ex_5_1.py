...
# print len of each parameters to verify
print("Number of agents:", len(agents))
print("Number of states per agent:", n)
print("len Formation graph G:", len(G))
print("len Formation references:", len(formation_references))
...
# %% Plot position error norm vs time
fig_error, ax_error = plt.subplots(num=22, clear=True, figsize=(10, 6))
ax_error.set_xlabel("Time [s]")
ax_error.set_ylabel("Position Error Norm [m]")
ax_error.set_title(f"Position Tracking Error ({wind_status})")
ax_error.grid(True, alpha=0.3)

for idx in range(len(agents)):
    # Compute position error norm for each time step
    error_norms = []
    for i, ti in enumerate(t):
        # Current position
        p_current = x[i, 0 + n * idx:2 + n * idx]
        # Reference position at time ti
        p_ref = np.array(formation_references[idx](ti)[0:2])
        # Compute error norm
        error_norm = np.linalg.norm(p_current - p_ref)
        error_norms.append(error_norm)
    
    ax_error.plot(t, error_norms, color=colors[idx], label=f'Agent {idx}', linewidth=2)

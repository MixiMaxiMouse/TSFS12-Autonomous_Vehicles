...
class MinAPDE(Metric):
    ...
    def update(
        ...
        if pred.dim() == 4:
            pred, _ = filter_prediction(pred, trg, mask, prob, min_criterion, best_idx, mode_first=mode_first)
        ...
        if mask is not None:
            mask_exp = mask.unsqueeze(1).repeat(1, seq_len, 1) & mask.unsqueeze(-1).repeat(1, 1, seq_len)  # (N, T, T)
